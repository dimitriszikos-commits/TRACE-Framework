...
print("="*85)
print("CLINICAL SEQUENCE EXTRACTOR: THE MASTER PIPELINE (SUPERVISED EDITION)")
print("="*85)
...
# 3. PRE-PROCESSING
# ==========================================
print("\nPHASE 2: Jaccard Bundling & Conditioned Negatives")
...
def mine_top_sequences(max_len, max_prune):
    ...
    P_target_baseline = y_train_bool.sum() / total_train
    if P_target_baseline == 0: P_target_baseline = 0.0001
    
    def calculate_outcome_lift(mask):
        seq_count = mask.sum()
        if seq_count == 0: return 1.0
        
        target_hits = np.bitwise_and(mask, y_train_bool).sum()
        P_target_given_seq = target_hits / seq_count
        
        if P_target_given_seq == 0: return P_target_baseline / 0.0001
        return max(P_target_given_seq / P_target_baseline, P_target_baseline / P_target_given_seq)
    ...
    for node_A, node_B in combinations(valid_nodes, 2):
        ...
        outcome_lift = calculate_outcome_lift(intersect_mask)
        
        if divergence >= div_threshold:
            markov = (P_B_g_A - P_marginal_train[node_B]) / P_marginal_train[node_B]
            power_score = (divergence**W_DIV) * (abs(markov)**W_MARK) * (outcome_lift**W_OUT)
            valid_sequences.append({'seq': (node_A, node_B), 'mask': intersect_mask, 'power': power_score})
        elif (1 / divergence) >= div_threshold:
            markov = (P_A_g_B - P_marginal_train[node_A]) / P_marginal_train[node_A]
            power_score = ((1/divergence)**W_DIV) * (abs(markov)**W_MARK) * (outcome_lift**W_OUT)
        ...
        for seq_dict in current_level:
            ...
            for next_node in valid_nodes:
                ...
                if div >= div_threshold and markov_passes:
                    outcome_lift = calculate_outcome_lift(new_mask)
                    power_score = (div**W_DIV) * (abs(markov)**W_MARK) * (outcome_lift**W_OUT)
...
print(f"--> Extracted and Ranked top {len(top_500_seqs)} valid sequences based on Supervised Power Score.")
...
if run_mode == 'Y':
    ...
    best_auc, best_p, best_c = 0, 50, 0.1
    ...
    for p_limit in prune_options:
        ...
        for c_val in c_options:
            ...
            print(f"  > Testing Top {p_limit} Seqs | Lasso C: {c_val:.2f} -> AUC: {auc:.4f} (Lift: {auc - baseline_auc:+.4f})")
            
            if auc > best_auc: best_auc, best_p, best_c = auc, p_limit, c_val
                
    final_prune, final_c = best_p, best_c
...
if best_edges:
    ...
    print("\nDumping the Hybrid Test Matrix to CSV for inspection...")
    inspection_cols = [TARGET_COL] + valid_nodes + sequence_feature_names
    df_test[inspection_cols].copy().to_csv("INSPECTION_Test_Matrix.csv", index=False)
    ...
    if "HYBRID" in set_name:
        ...
        if source_model is not None:
            ...
            for idx, row in seq_winners.head(5).iterrows():
                ...
                print(f"      * Pathway: {'  --->  '.join(clinical_chain)} (OR: {odds_ratio:.2f})")
            ...
            function update(source) {
              ...
              function diagonal(s, d) {
                if (d.data && d.data.name && d.data.name.includes("(Recurrence)")) {
                    return `M ${s.y} ${s.x} A 160 160 0 0 1 ${d.y} ${d.x}`;
                }
            ...
            if source_model is not None and not seq_winners.empty:
                ...
                def insert_path_lasso(current_node, path, coef):
                    if not path:
                        or_val = np.exp(coef)
                        outcome_str = f"OUTCOME: {'Increased' if coef > 0 else 'Decreased'} Risk (OR: {or_val:.2f})"
                    ...
                    insert_path_lasso(target_child, path[1:], coef)

                for idx, row in seq_winners.iterrows():
                    insert_path_lasso(tree_data_lasso, row['Feature'].replace("SEQ_", "").split("_TO_"), row['Coefficient'])
