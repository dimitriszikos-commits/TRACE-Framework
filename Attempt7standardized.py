...
print("="*85)
print("CLINICAL SEQUENCE EXTRACTOR: STANDARDIZED ABLATION EDITION")
print("="*85)
...
# ---------------------------------------------------------
# ABLATION LOCK: Parameters forced to match Attempt 8
# ---------------------------------------------------------
run_mode = 'N'
manual_prune = MAX_PRUNE_SEARCH
manual_lasso = 0.1
print("\n[5] ABLATION PARAMETERS LOCKED: Grid Search Disabled.")
print(f"    -> Sequence Pruning Limit enforced at: {manual_prune}")
print(f"    -> Lasso Strictness (C) enforced at: {manual_lasso}")
...
# ==========================================
# 5. OPTIMIZATION & EVALUATION (STANDARDIZED)
# ==========================================
f_p, f_c = min(MAX_PRUNE_SEARCH, len(sequence_feature_names)), manual_lasso
print(f"\n--> Using Standardized Hyperparameters: Top {f_p} Sequences | Lasso C: {f_c}")
...
raw_ccsr_only = [c for c in valid_nodes if not c.startswith("NOT_") and not c.startswith("BUNDLE_")]

feature_sets = {
    "1. DUMB BASELINE (PrefixSpan Match)": raw_ccsr_only,
    "2. SMART BASELINE (Raw + NOT_ Nodes)": valid_nodes,
    "3. ENGINEERED SEQS (Sequences Only)": final_features,
    "4. TRUE HYBRID (Dumb Baseline + Sequences)": raw_ccsr_only + final_features,
    "5. FULL HYBRID (Smart Baseline + Sequences)": valid_nodes + final_features
}
...
for set_name, features in feature_sets.items():
    ...
    # 1. Prepare Data
    X_tr_final = df_train[features].copy()
    y_tr_final = y_train
    X_te_final = df_test[features].copy()
    
    # 2. NON-LINEAR DOSE BINARIZATION
    # Unroll the ordinal doses into pure binary flags just for the model hand-off
    dose_cols = [c for c in features if "_DOSE]" in c]
    if dose_cols:
        X_tr_final = pd.get_dummies(X_tr_final, columns=dose_cols).astype(np.int8)
        X_te_final = pd.get_dummies(X_te_final, columns=dose_cols).astype(np.int8)
        # Align columns to ensure the Test set perfectly matches the Train set format
        X_te_final = X_te_final.reindex(columns=X_tr_final.columns, fill_value=0)
    
    # 3. Standard Baseline Models
    ...
    lasso_cv.fit(X_tr_final, y_tr_final)
    ...
    print(f"  > Lasso LR PR-AUC:     {final_test_pr:.4f} (True Lift: {test_lift_pr:+.4f}) | Surviving Features: {surviving}/{len(X_tr_final.columns)}")
    
    # Dynamically map coefficients using the newly expanded columns
    if "ENGINEERED" in set_name:
        coef_df = pd.DataFrame({'Feature': X_tr_final.columns, 'Coefficient': lasso.coef_[0]})
    ...
    if "HYBRID" in set_name:
        ...
        if source_model is not None:
            ...
            for idx, row in seq_winners.head(5).iterrows():
                ...
                # Also clean off the new Dummy level indicator for display purposes if present
                clean_feature = re.sub(r'_Level_\d+', '', clean_feature)
            ...
            if source_model is not None and not seq_winners.empty:
                ...
                for _, row in seq_winners.iterrows():
                    ...
                    # Clean off Dummy indicator before passing to dashboard mapping
                    clean_f = re.sub(r'_Level_\d+', '', clean_f)
