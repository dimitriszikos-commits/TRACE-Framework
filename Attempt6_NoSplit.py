...
OUTPUT_FILE = "Divergence_Sequences_100PCT.csv"
...
MIN_PREVALENCE_FOR_NEGATIVE = 0.05  

print("="*85)
print("CLINICAL SEQUENCE EXTRACTOR: BASELINE COMPARISON EDITION (100% DATA OVERLAP)")
print("="*85)
...
# ==========================================
# 2. OVERRIDE: NO SPLIT (100% OVERLAP)
# ==========================================
print("\nPHASE 1: OVERRIDE - USING 100% OF DATA FOR TRAIN/VAL/TEST")
df_train = df.copy()
df_val = df.copy()
df_test = df.copy()
print(f"--> Data split bypassed. Training, Validation, and Testing will ALL occur on the exact same {len(df_train)} patient cohort.")
...
def mine_top_sequences(max_len, max_prune):
    ...
    # Helper 2: The Streamlined Logic Gate
    def is_logically_valid(curr_seq, next_node):
        ...
        # BAN IMMEDIATE CONTRADICTIONS ONLY
...
for i, seq in enumerate(top_seqs):
    m_tr = np.ones(total_train, dtype=bool)
    for node in seq:
...
for base_path, variants in families.items():
    if len(variants) > 1:
        ...
        for dim_idx in range(num_dimensions):
            ...
            if dim_col_name not in df_train.columns:
                df_train[dim_col_name] = 0
                    
            for col, vector in variants:
                dose_val = vector[dim_idx] if dim_idx < len(vector) else 0
                mask = (df_train[col] == 1) & (df_train[dim_col_name] < dose_val)
                df_train.loc[mask, dim_col_name] = dose_val
...
df_val = df_train.copy() # Sync identical sets
...
# ==========================================
# 7. FINAL EVALUATION MATRIX (100% OVERLAP)
# ==========================================
print("\n" + "="*85)
print("PHASE 4: 3x3 CLINICAL VALIDATION MATRIX (EVALUATED ON IDENTICAL 100% DATASET)")
print("="*85)
...
for set_name, features in feature_sets.items():
    ...
    # Do not concat train and val to avoid artificially doubling the patient count
    ...
    if "HYBRID" in set_name:
        ...
        if source_model is not None:
            ...
            if source_model is not None and not seq_winners.empty:
                ...
                with open("DASHBOARD_1_Outcome_Drivers_100PCT.html", "w") as f: f.write(html_lasso)
                print("--> SUCCESS: Saved 'DASHBOARD_1_Outcome_Drivers_100PCT.html'")
            ...
            if best_edges:
                ...
                with open("DASHBOARD_2_All_Validated_Pathways_100PCT.html", "w") as f: f.write(html_markov)
                print("--> SUCCESS: Saved 'DASHBOARD_2_All_Validated_Pathways_100PCT.html'")
